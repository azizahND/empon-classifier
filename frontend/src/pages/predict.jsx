...
function Predict() {
  ...
  return (
    <>
      ...
      <div className="container mx-auto max-w-5xl px-4 py-10 text-center bg-whitemilk/40 shadow-md rounded-lg mt-8 mb-10 ">
        ...
        {imagePreview && (
          <div className="mb-6">
            ...
            <img 
              src={imagePreview} 
              alt="Preview" 
              className="mx-auto max-w-xs rounded-lg border transform transition duration-300 hover:scale-105 hover:shadow-lg"
            />
          </div>
        )}
        ...
        {prediction && (
          <div className="mt-6 text-left">
            ...
            <p className="text-gray-900 bg-white p-4 rounded-lg">{description}</p>

            <div className="flex flex-wrap gap-2 mt-4 justify-between">
              {detail.map((item, idx) => (
                <div 
                  key={idx} 
                  className="bg-greenDark text-white px-4 py-2 rounded-lg shadow-md"
                >
                  {item}
                </div>
              ))}
